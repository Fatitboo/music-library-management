...
export function EditTrackDialog({
  ...
  const [selectedGenre, setSelectedGenre] = useState(selectedTrack?.genre);
  const [fileName, setFileName] = useState({
    fileName: selectedTrack?.fileName,
    playbackUrl: selectedTrack?.playbackUrl,
    mp3Url: selectedTrack?.mp3Url,
  });
  const [fileAvt, setFileAvt] = useState(selectedTrack?.thumnailUrl);
  ...
  const onSubmitForm = async (data) => {
    ...
    await axios.put(
      "http://localhost:3000/tracks/" + selectedTrack?._id,
      dataPost
    );
    handleOpen();
  ...
  return (
    <>
      <Dialog size="sm" open={open} handler={handleOpen} className="p-4 z-0">
          ...
          <DialogBody className="space-y-4 pb-6">
            ...
            <div className="flex gap-4">
              <div className="w-full">
                <TextInput
                  value={selectedTrack?.releaseYear}
                  name="releaseYear"
                  type={"number"}
